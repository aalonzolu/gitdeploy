...
define("REMOTE_REPOSITORY", "git@host.org:user/projects.git");
define("DIR", "/var/www/project/");
define("BRANCH", "refs/heads/master"); // github gitlab
//define("BRANCH", "master"); // bitbucket
define("LOGFILE", "./deploy-project.log");
define("GIT", "/usr/bin/git");
define("AFTER_PULL", "");


// No modificar nada a partir de acá ¿ok?
...
*/
if(empty($json["ref"])){
  $json["ref"] = $json['push']['changes'][0]['new']['name'];
}
...
// if none of the above match, but a token exists, exit
    ...
    if ($json["ref"] === BRANCH) {
        ...
        fputs($file, "=== ERROR: Pushed ".$json["ref"]." does not match BRANCH ".BRANCH." ===\n");
    }
}
